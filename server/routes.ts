...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/uploadReport', requireAuth, async (req, res) => {
    try {
      ...
      const existingSharedReports = await storage.getSharedReportsByPatientId(patientId);
      const duplicateReport = existingSharedReports.find(sr => sr.reportId === reportId);
      
      if (duplicateReport) {
        console.log('⚠️ Duplicate upload attempt detected - returning existing shared report');
        ...
        if (existingDoctor) {
          return res.json({
            ...
            aiDetection: {
              detectedSpecialization: duplicateReport.detectedSpecialization || 'General Physician',
              confidence: 'high',
            },
            reportDetails: {
              patientId: patientId,
      ...
      if (!reportText) {
        return res.status(400).json({ 
          message: 'Report text could not be extracted. Please ensure the file is readable.',
        });
      }
